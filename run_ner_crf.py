...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    eval_dataset, token = load_and_cache_examples(args, args.task_name, tokenizer, data_type='dev')
    ...
    f = open(args.output_dir + 'result.txt', 'a', encoding='utf-8')
    results = {f'{key}': value for key, value in eval_info.items()}
    results['loss'] = eval_loss
    logger.info("***** Eval results %s *****", prefix)
    f.write("***** Eval results %s *****")
    info = "-".join([f' {key}: {value:.4f} ' for key, value in results.items()])
    f.write(info)
    logger.info(info)
    logger.info("***** Entity results %s *****", prefix)
    f.write("***** Entity results %s *****")
...
def predict(args, model, tokenizer, prefix=""):
    ...
    lines = DataProcessor._read_text(os.path.join(args.data_dir, "test.char"))
    pred_output_dir = args.output_dir
    if not os.path.exists(pred_output_dir) and args.local_rank in [-1, 0]:
        os.makedirs(pred_output_dir)
    test_dataset, token = load_and_cache_examples(args, args.task_name, tokenizer, data_type='test')
    # Note that DistributedSampler samples randomly
    test_sampler = SequentialSampler(test_dataset) if args.local_rank == -1 else DistributedSampler(test_dataset)
    test_dataloader = DataLoader(test_dataset, sampler=test_sampler, batch_size=args.test_batch_size, collate_fn=collate_fn)
    # Eval!
    ...
    for step, batch in enumerate(test_dataloader):
        ...
        with torch.no_grad():
            ...
            logits = outputs[0]
            tags = model.crf.decode(logits, inputs['attention_mask'])
            score, _, tag = model.crf.viterbi_decode2(logits, inputs['attention_mask'])
            tags = tags.squeeze(0).cpu().numpy().tolist()
            _ = _.squeeze(0).cpu().numpy().tolist()
            x = [round(i, 2) for i in list(_)]
        ...
        json_d['seq'] = ''.join(lines[step]['words'])
        json_d['p'] = list(x)
...
def load_and_cache_examples(args, task, tokenizer, data_type='train'):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        features, token = convert_examples_to_features(examples=examples,
                                                       tokenizer=tokenizer,
...
def main():
    ...
    # Training
    if args.do_train:
        train_dataset, token = load_and_cache_examples(args, args.task_name, tokenizer, data_type='train')
